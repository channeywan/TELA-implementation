...
class UnifiedTransformerModel(business_type_classify):
    ...
    def plot_history(self, trainer):
        ...
        plt.savefig(os.path.join(
            save_dir, f'{self.model_checkpoint}_loss_curve_{time_str}.png'))
        logger.info(
            f"Loss curve saved to {os.path.join(save_dir, f'{self.model_checkpoint}_loss_curve_{time_str}.png')}")
    ...
    def plot_confusion_matrix(self, trainer, test_dataset):
        ...
        plt.savefig(os.path.join(DirConfig.TEMP_DIR,self.model_checkpoint,
                    f'{self.model_checkpoint}_confusion_matrix.png'))
        logger.info(
            f"混淆矩阵已保存为 {os.path.join(DirConfig.TEMP_DIR,self.model_checkpoint,f'{self.model_checkpoint}_confusion_matrix.png')}")
    def set_logger(self):
    ...
    def load_trained_model(self):
        model_path=os.path.join(DirConfig.MODEL_DISTILL_DIR, self.model_checkpoint)
