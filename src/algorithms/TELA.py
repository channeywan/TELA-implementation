...
class TELA(BaseAlgorithm):
    ...
    def train_models(self):
        ...
        # 加载训练数据
        items_train_encoded = self.encode_item(self.train_items.copy())
    ...
    def train_cluster_classifier(self, items: pd.DataFrame):
        ...
        model_classify_stable = GridSearchCV(
            estimator=DecisionTreeClassifier(criterion="gini"),
            param_grid=params_grid, cv=3, scoring='accuracy', n_jobs=1, verbose=0
        )
    ...
    def load_item_predict_models(self, train_models: bool = True):
        ...
        if train_models:
            self.train_models()
            return self.load_item_predict_models(train_models=False)
        try:
            # 加载第一阶段分类器
